...
void Equirec2Perspec::setParams(const cv::Mat &inputImg, float FOV, int height, int width) {
...
void Equirec2Perspec::equirectangular2perspective(const cv::Mat &inputImage, cv::Mat &outputImage,
...
void Equirec2Perspec::equirectangular2perspective(const cv::Mat &inputImage, cv::Mat &outputImage,
    ...
    if(isSetParam_){
        ...
        std::cout << "Please set parameters before call equirectangular2perspective()" << std::endl;
        exit(1);
    }
}
...
void Equirec2Perspec::updateParams() {
    ...
    wFOV_ = FOV_;
    hFOV_ = float(perHeight_) / perWidth_ * wFOV_;
...
void Equirec2Perspec::equi2per(const cv::Mat &inputImage, cv::Mat &outputImage) {
    ...
    for(int li=0; li<idx1Cols; li++){
        if(idx1.at<float>(li)>0){ idx1.at<float>(li) = 1; }
        else{ idx1.at<float>(li) = 0; }
    }

    cv::Mat idx2 = xyz_.col(1).t();
    int idx2Cols = idx1.cols;
    for(int li=0; li<idx2Cols; li++){
        if(idx2.at<float>(li)>0){ idx2.at<float>(li) = 1; }
        else{ idx2.at<float>(li) = 0; }
    }
